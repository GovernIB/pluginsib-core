...
public class PluginsManager {
    ...
    public static Object instancePluginByProperty(String propertyPlugin, String basePropertiesKey,
        ...
        // Valor de la Clau
        String className = properties != null ? properties.getProperty(propertyPlugin.trim())
                : System.getProperty(propertyPlugin.trim());
        if (className == null || className.trim().length() == 0) {
            log.error("No puc trobar valor per la propietat '" + propertyPlugin + "' dins de les properties "
                    + properties);
            return null;
        }
    ...
    public static Object instancePluginByClassName(String className, String basePropertiesKey,
        ...
        try {
            ...
            final String msg = "Error carregant la classe " + className + " associada a un plugin:" + ex.getMessage();
            log.error(msg, ex);
            return null;
        }
    ...
    public static Object instancePluginByClass(Class<?> c, String basePropertiesKey, java.util.Properties properties) {
        // Instanciant la classe
        Object pluginInstance;
        try {
            if (!AbstractPluginProperties.class.isAssignableFrom(c)) {
                if (log.isDebugEnabled()) {
                    log.debug("instancePluginByClassName => " + c.getName() + " no es AbstractPluginProperties");
                }
                ...
                if (properties == null && (basePropertiesKey == null || basePropertiesKey.trim().length() == 0)) {
                    ...
                    try {
                        pluginInstance = c.getConstructor(String.class, Properties.class)
                                .newInstance(basePropertiesKey.trim(), properties);
            ...
            final String msg = "Error instanciant la classe " + c.getName() + " associada a un plugin:"
                    + e.getMessage();
            log.error(msg, e);
            return null;
        }
